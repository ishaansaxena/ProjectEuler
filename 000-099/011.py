...
# Row-wise
for i in range(constraint):
	for j in range(constraint-3):
		s = matrix[i][j] * matrix[i][j+1] * matrix[i][j+2] * matrix[i][j+3]
		if m < s:
			m = s
			n = [i, j, 0]
...
for i in range(constraint):
	for j in range(constraint-3):
		s = matrix[j][i] * matrix[j+1][i] * matrix[j+2][i] * matrix[j+3][i]
		if m < s:
			m = s
			n = [i, j, 1]

# Diagonals (Right)
for i in range(constraint-3):
	for j in range(constraint-3):
		s = matrix[j][i] * matrix[j+1][i+1] * matrix[j+2][i+2] * matrix[j+3][i+3]
		if m < s:
			m = s
			n = [i, j, 2]
